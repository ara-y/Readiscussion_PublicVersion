...
public class DiscussionService implements CallDiscussionService{
	...
	public void registerReadTime(Authentication authentication, String roomURL) {
		...
		//authenticationを直接ダウンキャストしSiteUserを取り出すことはできない？
		String userName = authentication.getName();
	...
	@Override
	public void registerRoom(String endDiscussion,  boolean hasPassword, String password, String roomName, String roomURL,SiteUser user, Room saveRoom, ForRoom room) throws ParseException {
	...
	public void autoTransferRoomAdminPrivilege(SiteUser user) {
		...
		for(int i=0; i< numberOfHostingRoom; i++) {
			...
			if(hostingRoom.getParticipatingUsers().isEmpty() != true) {

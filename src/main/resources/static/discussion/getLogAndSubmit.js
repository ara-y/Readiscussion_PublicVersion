/**
 *  403がでて送信できない。_csrfが取得できず。HTMLでModelAttributeされる前にJavaScriptが実行されている模様 →　HTMLにてvalueからth:value追加で解決済
 	・無限スクロールでのログ取得
 	・二重投稿防止
...
[]
[{"id":1,"message":"XXいいよね"},{"id":1,"message":"ジョーカーゲーム読み始めました"},]
[{"id":1,"message":"ジョーカーゲーム読み始めました"},{"id":1,"message":"XXいいよね"},{"id":1,"message":"ジョーカーゲーム読み始めました"}]
と連続で取得されてDOMexceptionが発生した。


null判定　配列→null→空の配列のみが送られる　SpringBoot側では空の配列→配列→配列→空の配列
 */
